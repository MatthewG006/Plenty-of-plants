...
export default function GardenPage() {
  ...
  const [gardenPlantIds, setGardenPlantIds] = useState<(number | null)[]>(Array(NUM_POTS).fill(null));
  ...
  useEffect(() => {
    if (gameData) {
        setGardenPlantIds(gameData.gardenPlantIds || Array(NUM_POTS).fill(null));
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (activeSource === 'collection') {
      if (overType === 'pot') {
        ...
        const plantInPotId = newGardenIds[potIndex];

        // Add dragged plant to pot
        newGardenIds[potIndex] = activePlant.id;
    ...
    // Dragging from garden
    else if (activeSource === 'garden') {
      const sourcePotIndex = newGardenIds.findIndex(id => id === activePlant.id);

      if (sourcePotIndex === -1) return;

      if (overType === 'pot') { // garden to garden
        const targetPotIndex = parseInt(overIdStr, 10);
        const plantInTargetPotId = newGardenIds[targetPotIndex];
        
        // Swap plants
        newGardenIds[targetPotIndex] = activePlant.id;
        newGardenIds[sourcePotIndex] = plantInTargetPotId;

      } else if (overType === 'collection') { // garden to collection
        newGardenIds[sourcePotIndex] = null;
    ...
    setGardenPlantIds(newGardenIds);
    setCollectionPlantIds(finalCollectionIds);
    await updatePlantArrangement(user.uid, finalCollectionIds, newGardenIds);
  };
    ...
    >
      <div className="space-y-4 bg-white min-h-screen">
        <header className="flex flex-col items-center gap-4 p-4 text-center">
          <h1 className="text-3xl text-primary text-center">My Garden</h1>
        </header>
