...
function DraggablePlant({ plant, source, canWater, ...rest }: { plant: Plant; source: 'desk'; canWater: boolean; } & React.HTMLAttributes<HTMLDivElement>) {
...
export default function GardenPage() {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const newCollectionIds = [...gameData.collectionPlantIds];
    ...
    if (overType === 'pot') { // desk to desk, or collection to desk
        ...
        if (activeSource === 'desk') {
         ...
         const finalCollectionIds = gameData.collectionPlantIds.filter(id => id !== activePlant.id);
         if (plantInPotId) {
    ...
    >
      <div className="space-y-4 bg-white min-h-screen">
        <header className="flex flex-col items-center gap-2 p-4 text-center">
          ...
          <p className="text-muted-foreground text-sm">Drag plants from your collection into the plots to display and water them.</p>
        </header>
        ...
        <section className="px-4">
            ...
            <div className="relative max-w-lg mx-auto">
                <Image
                    ...
                    priority
                />
                <div className="absolute inset-0 grid grid-cols-3 grid-rows-4 p-4 pt-10 pb-56 sm:p-6 sm:pt-12 sm:pb-56 md:p-8 md:pt-14 md:pb-56 gap-x-6 gap-y-0">
