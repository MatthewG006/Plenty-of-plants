...
function PlantCard({ plant, onClick, processedImage }: { plant: Plant, onClick: (plant: Plant) => void, processedImage: string | null }) {
    return (
        <Card className="group overflow-hidden shadow-md w-full h-[110px] sm:h-[130px] relative cursor-pointer bg-white/70 backdrop-blur-sm" onClick={() => onClick(plant)}>
...
function EmptyPlotCard({ onClick }: { onClick: () => void }) {
    return (
        <Card className="group overflow-hidden shadow-md w-full h-[110px] sm:h-[130px] relative cursor-pointer bg-black/10 backdrop-blur-sm" onClick={onClick}>
            <CardContent className="p-0 flex flex-col h-full">
