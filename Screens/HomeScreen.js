...
// Sample free plants provided daily
const freePlants = [
  {
    id: '1',
    name: 'Cactus',
...
const HomeScreen = ({ navigation }) => {
  ...
  // Load the last claimed date from AsyncStorage
  ...
  const handleClaimPlants = async () => {
    ...
    // Add free plants to the player's collection
    setPlants([...plants, ...freePlants]);
    await AsyncStorage.setItem('lastClaimed', today.toISOString());
    setLastClaimed(today);
  };

  const renderPlant = ({ item }) => (
    <PlantCard plant={item} onPress={() => navigation.navigate('PlantDetail', { plant: item })} />
  );
  ...
  return (
    <View style={styles.container}>
      ...
      />
      <Text style={styles.instructions}>Tap on a plant to interact, water, or upgrade!</Text>
    </View>
  );
};
